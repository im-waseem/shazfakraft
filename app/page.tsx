...
export default function Home() {
  ...
  const announcementTexts = banners.length > 0
    ...
    : ['Free shipping on orders over ₹499', 'Authentic Islamic products', 'New arrivals every week', 'Trusted by 5000+ customers']
    ...
    <div style={{ minHeight: '100vh', background: '#fffbf5', fontFamily: "'Nunito', sans-serif", color: '#1c1410' }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        :root{
          --cream:#fffbf5;--warm:#fdf3e3;--gold:#c8860a;--gold-light:#e8a020;
          --gold-pale:#fef3d8;--sage:#6b7c5c;--brick:#c04e2a;--text:#1c1410;
          --text-2:#5a4a3a;--text-3:#9a8a7a;--border:#ecdcc8;--border-2:#dccaaa;
        ...
        .cat-card{background:#fff;border:1px solid var(--border);border-radius:var(--radius);padding:20px 16px;text-align:center;cursor:pointer;transition:all .25s;text-decoration:none;color:var(--text);border:none;width:100%}
        ...
        @media(max-width:768px){.desktop-sidebar{display:none!important}.main-grid{grid-template-columns:1fr!important}.hide-mobile{display:none!important}}
      ...
      <header style={{ background: '#fff', borderBottom: '1px solid var(--border)', position: 'sticky', top: 0, zIndex: 30, boxShadow: '0 2px 12px rgba(200,134,10,.06)' }}>
        <div style={{ maxWidth: 1320, margin: '0 auto', padding: '0 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', height: 64, gap: 20 }}>
            <button onClick={() => setMobileMenuOpen(p => !p)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 6, color: 'var(--text)', display: 'none' }} className="hide-desktop">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <Link href="/wishlist" style={{ position: 'relative', color: 'var(--text-2)', display: 'flex', textDecoration: 'none', padding: '8px', borderRadius: 10 }}>
              ...
              <Link href="/cart" style={{ position: 'relative', color: 'var(--text-2)', display: 'flex', textDecoration: 'none', padding: '8px', borderRadius: 10 }}>
              ...
              {user ? (
                ...
                <Link href="/login" style={{ background: 'var(--gold)', color: '#fff', padding: '9px 18px', borderRadius: 50, fontSize: 13, fontWeight: 700, textDecoration: 'none', letterSpacing: '.03em' }}>Sign In</Link>
              )}
            </div>
          </div>
        </div>
        <div className={`mobile-menu ${mobileMenuOpen ? 'open' : ''}`}>
          <nav style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {['/', '/products', '/about', '/contact'].map((href, i) => (
              <Link key={href} href={href} onClick={() => setMobileMenuOpen(false)} style={{ color: 'var(--text)', fontWeight: 600, fontSize: 15, textDecoration: 'none', padding: '10px 8px', borderRadius: 8 }}>
      ...
      <section style={{ background: 'linear-gradient(135deg,#fffbf5 0%,#fdf3e3 60%,#fef3d8 100%)', padding: 'clamp(40px,6vw,80px) 20px clamp(32px,4vw,56px)', textAlign: 'center', borderBottom: '1px solid var(--border)', position: 'relative', overflow: 'hidden' }}>
        ...
        <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(2.2rem,5vw,4rem)', fontWeight: 700, color: 'var(--text)', lineHeight: 1.1, marginBottom: 8 }}>
          Discover Authentic<br /><span style={{ color: 'var(--gold)', fontStyle: 'italic' }}>Islamic Products</span>
        </h1>
        <p style={{ color: 'var(--text-2)', fontSize: 'clamp(14px,2vw,16px)', maxWidth: 480, margin: '10px auto 36px' }}>
      ...
      {categories.length > 0 && !searchQuery && selectedCategories.length === 0 && (
        <section style={{ maxWidth: 1320, margin: '0 auto', padding: '40px 20px 0' }}>
          <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.4rem,3vw,2rem)', fontWeight: 700, marginBottom: 20 }}>Shop by Category</h2>
      ...
      <div className="main-grid" style={{ maxWidth: 1320, margin: '0 auto', padding: '32px 20px 60px', display: 'grid', gridTemplateColumns: '240px 1fr', gap: 32, alignItems: 'start' }}>
        ...
        <div className={`mobile-sidebar ${mobileSidebarOpen ? 'open' : ''}`}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            ...
            <button onClick={() => setMobileSidebarOpen(false)} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer' }}>×</button>
          </div>
          <SidebarContent {...{ categories, selectedCategories, toggleCategory, priceRange, setPriceRange, maxPrice, hasFilters, clearFilters }} />
        </div>
        <main>
                  ...
                  <div key={product.id} className="p-card">
                    <Link href={`/products/${product.slug}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
                      ...
                      <div style={{ padding: '12px 14px 6px' }}>
                        ...
                        <p style={{ fontSize: 14, fontWeight: 600, marginTop: 4, lineHeight: 1.35, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{product.name}</p>
      ...
      <footer style={{ background: '#1c1410', color: '#e8d5b0', padding: '52px 20px 28px', marginTop: 20 }}>
        <div style={{ maxWidth: 1320, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 36, marginBottom: 40 }}>
            <div>
              <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 28, fontWeight: 700, color: '#d4a843', marginBottom: 10 }}>Shazfa kraft</div>
          ...
          <div style={{ borderTop: '1px solid rgba(255,255,255,.08)', paddingTop: 20, display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
            <p style={{ color: '#6a5a4a', fontSize: 12 }}>© {new Date().getFullYear()} Shazfa kraft Islamic Store. All rights reserved.</p>
