...
/* ─── Build image list ────────────────────────────────────────────────────────
   Merges product images + unique variant images into one deduplicated array.
   Returns both the flat list and a colorImageIndex map (color → index).
─────────────────────────────────────────────────────────────────────────────*/
...
const ProductReviews = lazy(() => import('@/components/ProductReviews'))
  ...
  const fetchProduct = useCallback(async () => {
    ...
    if (data) {
      ...
      if (vList.length > 0) {
        ...
        // Jump gallery to the first variant's color image if it has one
        // (will be resolved after product+variants are in state via effect below)
      }
      const relQ = supabase
        .from('products')
      ...
      const { data: rel } = data.category_id
        ? await relQ.eq('category_id', data.category_id)
        : await relQ
  ...
  const handleColorSelect = (color: string) => {
    ...
    // ── KEY FIX: jump gallery to this color's image index ──
  ...
  const goToImage = (index: number) => {
    ...
    // ── KEY FIX: when user manually slides, update selectedColor to match
    //    that image's variant color (if any variant owns this image)
  ...
  /* ── The current display image: just use allImages[selectedImage] ─────────
     No more separate "colorImage" override — the gallery already contains
     the variant image at the right index, and goToImage/handleColorSelect
     keep selectedImage in sync with the selected color.
  ─────────────────────────────────────────────────────────────────────────*/
  ...
  if (!product) return (
    <div style={{ minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, fontFamily: 'sans-serif' }}>
      ...
      <Link href="/" style={{ background: '#b8860b', color: '#fff', padding: '10px 26px', borderRadius: 10, textDecoration: 'none', fontWeight: 700, fontSize: 14 }}>
        ← Back to Home
      </Link>
    </div>
  )
